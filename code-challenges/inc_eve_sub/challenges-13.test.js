...
const firstLetters = (arr) => {
  // Solution code here...
  let newArr = [];
  arr.forEach(firstLetter => {
    const letter = firstLetter.charAt(0)
    newArr.push(letter)
  });
  return newArr;
};
...
const findEvery = (arr, target) => {
  // Solution code here...
  const contains = arr.every(el => {
    return el.indexOf(target) > -1;
  })
