...
const count = (target, input) => {
  // Solution code here...
  const reduced = input.reduce((accumulator, valueArr) => {
    accumulator += valueArr.filter(el => {
      return el === target;
    }).length;
    return accumulator;
  },0);
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  const mapped = input.map(arr => {
    return arr.filter(el => {
      return !isNaN(el) && Number (el) === el && el % 5 === 0;
    }).map(el => {
...
let findShortest = (data) => {
  // Solution code here...
  const mappedStr = data.reduce((accumulator, value) => {
    if (Number(accumulator.height) > Number(value.height)){
      accumulator = value;
    }
    return accumulator;
  })
  return mappedStr.name;
};
